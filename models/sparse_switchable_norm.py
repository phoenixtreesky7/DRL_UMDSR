...
class SSN2d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.96, using_moving_average=True, last_gamma=False):
...
class SSN2d_LI(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.9, using_moving_average=True, last_gamma=False, using_bn = False):
        super(SSN2d_LI, self).__init__()
        self.eps = eps
        self.momentum = momentum
        self.using_moving_average = using_moving_average
        self.last_gamma = last_gamma
    ...
    def forward(self, x):
        ...
        if self.using_bn:
            mean = mean_weight_[0] * mean_in + mean_weight_[1] * mean_ln + mean_weight_[2] * mean_bn
            var = var_weight_[0] * var_in + var_weight_[1] * var_ln + var_weight_[2] * var_bn
        else:
            mean = mean_weight_[0] * mean_in + mean_weight_[1] * mean_ln
            var = var_weight_[0] * var_in + var_weight_[1] * var_ln
